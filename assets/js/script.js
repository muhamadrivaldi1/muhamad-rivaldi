...
// fullscreen image (zoom) modal — shared by Portfolio and Certificate sections
...
const zoomableLinks = document.querySelectorAll(".project-item a, .certificate-posts a");

const openPortfolioModal = function (imgSrc, imgAlt) {
  portfolioModalImg.src = imgSrc;
  portfolioModalImg.alt = imgAlt || "Modal Image";
  portfolioModalContainer.classList.add("active");
  portfolioOverlay.classList.add("active");
  document.body.style.overflow = "hidden"; // lock background scroll while fullscreen view is open
}
...
zoomableLinks.forEach(link => {
  link.addEventListener("click", function (e) {
    e.preventDefault();
    const img = this.querySelector("img");
    if (img) openPortfolioModal(img.src, img.alt);
  });
});

// close portfolio modal via close button, overlay click, or Escape key
